...
🙏🏻`;
  ...
  function buildValues(input){
    var now=new Date(),year=now.getFullYear(),month=now.getMonth(),data=getLocal().months[keyFor(now)]||{},current=rowsFor(year,month,data),last=lastRow(current),values={TOKO:'',TANGGAL:now.toLocaleDateString('id-ID')};
